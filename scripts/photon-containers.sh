...
CONTOUR_VERSION=$(jq -r < ${BOM_FILE} '.["contour"].gitRepoTag')
...
KNATIVE_VERSION=$(jq -r < ${BOM_FILE} '.["knative"].gitRepoTag')
...
RABBITMQ_OPERATOR_VERSION=$(jq -r < ${BOM_FILE} '.["rabbitmq-operator"].gitRepoTag')
...
RABBITMQ_BROKER_VERSION=$(jq -r < ${BOM_FILE} '.["rabbitmq-broker"].gitRepoTag')
